...
RegisterNetEvent("ardelivery:start_cooldown")
AddEventHandler("cardelivery:start_cooldown", function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	cdCooldown = true
	TriggerClientEvent("cardelivery:update-cooldown", -1, cdCooldown)

	if Player.PlayerData.metadata[metaDataName] ~= nil then
		TriggerClientEvent("cardelivery:client-receive-rank", src, Player.PlayerData.metadata[metaDataName])
	else
		TriggerClientEvent("cardelivery:client-receive-rank", src, 0)
	end
...
RegisterNetEvent("cardelivery:cooldown-request")
AddEventHandler("cardelivery:cooldown-request", function()
	local src = source
	TriggerClientEvent("cardelivery:update-cooldown", src, cdCooldown)
end)

RegisterNetEvent("cardelivery:request-cooldown-time")
AddEventHandler("cardelivery:request-cooldown-time", function()
	TriggerClientEvent("QBCore:Notify", source, (text_cooldown .. " - " .. secondsLeft .. " " .. text_secondsLeft), "primary", 3000)
end)

RegisterNetEvent("cardelivery:addMoney")
AddEventHandler("cardelivery:addMoney", function(amount)
	local src = source
	if payInCash then
		local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("cardelivery:GetMetaData")
AddEventHandler("cardelivery:GetMetaData", function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("-cardelivery:SetMetaData", function(meta, data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player then
		Player.Functions.SetMetaData(meta, data)
		TriggerClientEvent("cardelivery:client-receive-rank", src, data)
	end
end)
